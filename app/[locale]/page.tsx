        ...
        <div className="flex flex-wrap gap-3 items-center">
          {SORT_OPTIONS.map((option) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-base font-semibold shadow bg-blue-500 text-white hover:bg-blue-600 transition flex items-center gap-1 ${sort === option.value ? "" : "opacity-80"}`}
            >
              {option.label}
              {sort === option.value && (
                <span>
        ...
        {items.map((item) => (
          <Link key={item.id} href={`/items/${item.id}`} className="group">
            <div className="border rounded-lg shadow-md bg-white p-4 hover:shadow-lg transition-shadow duration-300 flex flex-col">
              ...
              <p className="text-base text-green-600 font-bold mb-1">{t('currency')}{item.price}</p>
