...
export default async function LoginUser({ params }: { params: Promise<{ locale: string }> }) {
  ...
  async function loginUser(formData: FormData) {
    ...
    try {
      ...
      let redirectUrl = formData.get('redirectAfterLogin') as string || `/${locale}`;
      if (redirectUrl.includes('/auth/signin') || redirectUrl.includes('/users/login')) {
        redirectUrl = `/${locale}`;
      }
  ...
  return (
    <div className="max-w-sm w-full mx-auto p-3 sm:p-6 bg-white rounded-lg shadow-md mt-2 mb-16 min-h-screen flex flex-col justify-start">
      ...
      <Form action={loginUser} className="space-y-4 sm:space-y-6">
        <input type="hidden" name="redirectAfterLogin" value={typeof window !== 'undefined' ? (localStorage.getItem('redirectAfterLogin') || '') : ''} />
        <div>
          <label
            htmlFor="username"
            className="block text-base sm:text-lg font-medium mb-1 sm:mb-2 text-gray-700"
          >
            {t('username_or_email')}
          </label>
          <input
